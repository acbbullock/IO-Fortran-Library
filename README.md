...
* `decimals` is `optional` and of type `int32`, which prescribes the number of decimal places to write.
...
For arrays of rank greater than `2`, only binary writes are allowed, so the file specified must have an extension equal to one of `binary_ext`.
...
Note that `data_shape` must be present when reading from binary files, and that the size of `data_shape` must match the rank of `into` for the read to be valid. The subprogram will not allow for a rank-shape mismatch of this kind and will `error stop` the program if such a mismatch is present, due to the critical nature of reads.
...
* `fmt` is `optional`, may be one of `['i', 'z']` for `integer` data or one of `['e', 'f', 'z']` for `real` or `complex` data, which specifies the text format of the data.
* `decimals` is `optional` and of type `int32`, which prescribes the number of decimal places to display.
...
This type is a simple wrapper for an allocatable string `s`, provided for the purpose of declaring arrays of varying-length strings whenever such an object is preferred over an array of identical length strings.
